...
async def test_hmip_dimmer(
    ...
    mock_hap = await default_mock_hap_factory.async_get_mock_hap(
        test_devices=["3014F711AAAA000000000005"]
    )
    ...
    mock_hap.runner.rest_connection.async_post.assert_called_with(
        ...
        {
            "deviceId": hmip_device.id,
            "channelIndex": 1,
            "dimLevel": 1.0,
        },
    )
    # assert mock_hap.runner.rest_connection.async_post.mock_calls[-1][1][1]['deviceId'] == hmip_device.id
    # assert mock_hap.runner.rest_connection.async_post.mock_calls[-1][1][1]['channelIndex'] == 1
    # assert mock_hap.runner.rest_connection.async_post.mock_calls[-1][1][1]['dimLevel'] == 1.0
...
async def test_hmip_wired_multi_dimmer(
    ...
    mock_hap = await default_mock_hap_factory.async_get_mock_hap(
        test_devices=["3014F711000WIREDDIMMER3"]
    )
...
async def test_hmip_din_rail_dimmer_3_channel3(
    ...
    mock_hap = await default_mock_hap_factory.async_get_mock_hap(
        test_devices=["3014F711A000DIN_RAIL_DIMMER3"]
    )
